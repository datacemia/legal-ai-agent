...
CONTRACT_DURATION_PATTERNS = {
    ...
    "fr": (
        r"\b(?:le\s+présent\s+|le\s+present\s+)?(?:accord|contrat)\s+(?:a|aura)\s+une\s+(?:durée|duree)\s+initiale\s+de\s+([^.;\n]{2,120})",
    ...
    "ar": (
        r"(?:تكون|تبلغ)\s+مدة\s+(?:هذه\s+)?(?:الاتفاقية|العقد)\s+الأولية\s+([^.;؛!؟\n]{2,120})",
