...
_JURISDICTION_CONTEXT_SIGNALS = [
    ...
    "مكان التحكيم",
    "يجري التحكيم في",
    "تعقد جلسات التحكيم في",
    "الاختصاص القضائي الحصري لمحاكم",
    "الاختصاص القضائي الحصري",
    "الاختصاص القضائي",
    "المحاكم الواقعة في",
    "المحاكم الكائنة في",
    "المحاكم الموجودة في",
    "المحاكم الولائية والفيدرالية",
    "المحاكم الفيدرالية والولائية",

    # Arabic -- governing law
    "يخضع هذا العقد لقانون",
    "يخضع هذا الاتفاق لقانون",
    "تخضع هذه الاتفاقية لقانون",
    "تخضع الاتفاقية لقانون",
    "تخضع لقانون",
    "يخضع لقانون",
    "تحكمه قوانين",
    "وفقاً لقوانين",
...
# ---------------------------------------------------------------------------
# V4.1.22 / V4.1.22A — organization contractual-actor gate.
#
# GLiNER may classify standards, frameworks, accounting/domain nouns, and
# other normative references as organizations. Redacting every ORGANIZATION
# / COMPANY prediction destroys substantive legal meaning (for example the
# NIST Cybersecurity Framework) and can alter downstream family detection.
#
# Organization/company spans are therefore pseudonymized only when the source
# supplies conservative evidence that the span is a real contractual actor:
#   1. a legal-entity suffix;
#   2. a defined contractual role immediately after the entity; or
#   3. actor syntax showing the entity performing/bearing a contractual act.
#
# The gate is multilingual (EN/FR/AR), domain-agnostic, and intentionally
# conservative. V4.1.22A adds narrow Arabic verb-subject-object support where
# a contractual actor follows an actor verb (e.g. "تقدم شركة ... الخدمات").
# ---------------------------------------------------------------------------

_ORGANIZATION_LEGAL_SUFFIX_PATTERN = re.compile(
    r"(?:"
    # English / international forms
    r"\b(?:llc|l\.l\.c\.|inc\.?|incorporated|corp\.?|corporation|"
    r"ltd\.?|limited|plc|lp|llp|gmbh|ag|bv|nv|oy|ab|as)\b"
    r"|"
    # French / civil-law forms
    r"\b(?:sas|sasu|sarl|sa|s\.a\.|sci|scs|sca|eurl|snc)\b"
    r"|"
    # Arabic company forms / common Arabic renderings
    r"(?:ذ\.?\s*م\.?\s*م\.?|ش\.?\s*م\.?\s*م\.?|ش\.?\s*م\.?\s*ع\.?|"
    r"شركة\s+ذات\s+مسؤولية\s+محدودة|شركة\s+مساهمة)"
    r")",
    re.IGNORECASE,
)


_ORGANIZATION_DEFINED_ROLE_PATTERN = re.compile(
    r"^\s*(?:,|\s)*\(\s*(?:the\s+|le\s+|la\s+|l['’]\s*|"
    r"ci-après\s+|hereinafter\s+)?"
    r"[\"'\u201c\u2018«]?"
    r"[^)\"'\u201d\u2019»]{0,60}"
    r"[\"'\u201d\u2019»]?\s*\)",
    re.IGNORECASE,
...
_ORGANIZATION_ACTOR_FOLLOWING_PATTERNS = (
    # English — entity/party as grammatical subject of a contractual act.
    re.compile(
        r"^\s*(?:,|\)|\]|\")*\s*"
        r"(?:shall|must|will|may|agrees?\s+to|undertakes?\s+to|"
        r"is\s+(?:required|obliged|obligated|responsible|entitled)|"
        r"has\s+(?:the\s+)?(?:right|obligation|duty)|"
        r"provides?|performs?|maintains?|notifies?|indemnifies?|"
        r"pays?|delivers?|supplies?|appoints?|engages?|processes?|"
        r"licenses?|assigns?|transfers?|represents?|warrants?)\b",
        re.IGNORECASE,
    ),
    # French.
    re.compile(
        r"^\s*(?:,|\)|\]|\")*\s*"
        r"(?:doit|devra|peut|s['’]engage\s+[àa]|est\s+(?:tenu|responsable|"
        r"autorisé|autorisee|autorisée)|a\s+(?:le\s+)?(?:droit|l['’]obligation)|"
        r"fournit|exécute|execute|maintient|notifie|indemnise|paie|livre|"
        r"nomme|engage|traite|concède|concede|cède|cede|transfère|transfere|"
        r"déclare|declare|garantit|respecte)\b",
        re.IGNORECASE,
    ),
    # Arabic — subject before verb.
    re.compile(
        r"^\s*(?:،|,|\)|\])*\s*"
        r"(?:يلتزم|تلتزم|يجب\s+على|يجوز\s+ل|يحق\s+ل|يتعهد|تتعهد|"
        r"يقدم|تقدم|يوفر|توفر|ينفذ|تنفذ|يحافظ|تحافظ|يخطر|تخطر|"
        r"يعوض|تعوض|يدفع|تدفع|يسلم|تسلم|يعين|تعين|يتعاقد|تتعاقد|"
        r"يعالج|تعالج|يرخص|ترخص|يتنازل|تتنازل|ينقل|تنقل|يقر|تقر|"
        r"يضمن|تضمن)\b",
        re.IGNORECASE,
    ),
)
...
_ARABIC_VSO_ACTOR_PREFIX_PATTERN = re.compile(
    r"(?:^|[\s.;؛!؟])"
    r"(?:يلتزم|تلتزم|يتعهد|تتعهد|يقدم|تقدم|يوفر|توفر|ينفذ|تنفذ|"
    r"يحافظ|تحافظ|يخطر|تخطر|يعوض|تعوض|يدفع|تدفع|يسلم|تسلم|"
    r"يعين|تعين|يتعاقد|تتعاقد|يعالج|تعالج|يرخص|ترخص|يتنازل|"
    r"تتنازل|ينقل|تنقل|يقر|تقر|يضمن|تضمن)"
    r"\s+(?:ال|شركة\s+|مؤسسة\s+|مصرف\s+|بنك\s+)?$",
    re.IGNORECASE,
)


def _has_organization_actor_evidence(
    text: str,
    start: int,
    end: int,
    value: str,
    ...
    """
    Conservative multilingual actor test for GLiNER organization/company
    predictions.

    This function does not decide whether a name is a famous institution or
    maintain a catalogue of standards. It relies on contract structure and
    grammar, so the behavior generalizes across contract families and domains.
    """
    normalized_value = normalize_spaces(value)

    if _ORGANIZATION_LEGAL_SUFFIX_PATTERN.search(normalized_value):
        return True

    after = text[end:min(len(text), end + 120)]
    before = text[max(0, start - 100):start]

    if _ORGANIZATION_DEFINED_ROLE_PATTERN.search(after):
        quoted_spans = re.findall(
            r"[\"'\u201c\u2018«]([^\"'\u201d\u2019»]{1,60})"
            r"[\"'\u201d\u2019»]",
            after[:100],
        )
        if any(tag_for_role(role_text, "") for role_text in quoted_spans):
            return True

    if any(
        pattern.search(after)
        for pattern in _ORGANIZATION_ACTOR_FOLLOWING_PATTERNS
    ):
        return True

    # V4.1.22A: narrow Arabic VSO support. The actor verb must be immediately
    # before the predicted entity (optionally followed by a generic company
    # designator). This avoids treating Arabic domain nouns elsewhere in the
    # sentence as parties merely because an unrelated verb exists nearby.
    if _ARABIC_VSO_ACTOR_PREFIX_PATTERN.search(before):
        return True
    ...
    for chunk_offset, chunk_text in split_into_gliner_chunks(text):
        ...
        for entity in entities:
            ...
            if label in {"organization", "company", "person"}:
                if (
                    label in {"organization", "company"}
                    and not _has_organization_actor_evidence(
                        text,
                        absolute_start,
                        absolute_end,
                        value,
                    ...
                    # Preserve standards, frameworks, normative references,
                    # and domain nouns unless the source itself supplies
                    # conservative evidence that the span is a contractual
                    # actor. Person entities remain subject to the existing
                    # PII behavior below.
                    continue
