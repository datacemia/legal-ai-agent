...
Public functions kept compatible with the previous file:
...
# Stable keys should be used in the database and API.
# Labels can be shown in the UI depending on user language.
# -----------------------------------------------------------------------------
...
# Backward-compatible aliases for old category names used by existing UI/charts.
CATEGORY_ALIASES = {
    "food": "food_dining",
...
# Priority matters. Specific financial operations should be detected before generic words.
CATEGORY_PRIORITY = [
    "income",
    "debt_loans",
    "savings_investments",
...
CATEGORY_KEYWORDS = {
    ...
    "shopping": [
        "pos purchase", "card purchase", "retail", "store", "shopping", "shop", "mall", "fashion",
        "clothes", "clothing", "electronics", "amazon", "aliexpress", "shein", "temu", "ikea",
    ...
    "debt_loans": [
        ...
        "personal loan", "mortgage payment", "finance payment",
    ...
    "transfers": [
        ...
        "remittance", "p2p", "family transfer", "friends", "beneficiary",
    ...
    "subscriptions": [
        ...
        "aws", "google", "youtube", "canva", "apple.com bill", "canal+", "deezer", "icloud",
        "microsoft", "office 365", "dropbox", "notion", "github", "figma", "zoom", "slack",
    ...
    "fees": [
        ...
        "service charge", "transfer fee", "atm fee",
    ...
    "cash": [
        "atm", "cash withdrawal", "withdrawal", "cash", "cash machine", "cashpoint", "cash dispenser",
...
# Merchant memory: local and global merchants that often appear without category words.
# Add new merchants here or load from DB later.
MERCHANT_CATEGORY_OVERRIDES = {
    # Saudi / Arabic examples from bank statements
    "ستاربكس": "food_dining",
    "starbucks": "food_dining",
    "ماكدونالدز": "food_dining",
    "mcdonald": "food_dining",
    "شاورمر": "food_dining",
    "باسكن روبنز": "food_dining",
    "بروست": "food_dining",
    "مدينه النعناع": "food_dining",
    "مدينة النعناع": "food_dining",
    "الدانوب": "groceries",
    "danube": "groceries",
    "تموينات": "groceries",
    "مون مارت": "groceries",
    ...
    "زهرة": "healthcare",
    "stc": "utilities",
    "اتصالات السعوديه": "utilities",
    "الكهرباء": "utilities",
    "شركة المياه": "utilities",
    "شركه المياه": "utilities",
    "محطه نفط": "transport",
    "محطة نفط": "transport",
    "بترول": "transport",
    "ورشه": "transport",
    ...
    "فندق": "travel",
    "مصاريف سفر": "travel",
    "المخالفات المروريه": "government_taxes",
    "المخالفات المرورية": "government_taxes",
    "قسط قرض": "debt_loans",
    "قرض شخصي": "debt_loans",
    "حواله فوريه": "transfers",
    "حوالة فورية": "transfers",
...
GENERIC_PAYMENT_WORDS = [
    ...
    r"\bpos\b", r"\bcard\s*purchase\b", r"\bpos\s*purchase\b",
    r"paiement\s*carte", r"achat\s*carte",
...
def normalize_text(text: str) -> str:
    """Normalize multilingual bank text without destroying Arabic merchant names."""
    if text is None:
        return ""
    ...
    # Remove common card/reference noise, screen numbers, long numeric IDs, masked cards.
    text = re.sub(r"\*+", " ", text)
    ...
    # Keep Arabic letters, Latin letters and useful separators.
...
def detect_category(description: str) -> str:
    ...
    3) Safe generic POS fallback -> shopping
    4) other
    """
    ...
    # 1. Merchant memory / overrides first.
    ...
    # 2. Priority keyword matching.
    for category in CATEGORY_PRIORITY:
        for keyword in CATEGORY_KEYWORDS.get(category, []):
            if _keyword_matches(keyword, normalized, tokens):
                return _canonical_category(category)

    # 3. Generic card purchase fallback.
    # Only use this when there is a purchase signal but no better merchant/category detected.
    lowered_original = original.lower()
    if re.search(r"pos purchase|card purchase|paiement carte|achat carte|شراء عبر نقاط بيع|نقاط بيع|مدي اثير|مدى أثير", lowered_original):
        return "shopping"
...
def category_is_cashflow_neutral(category: str) -> bool:
    """Transfers/savings are not always consumption; keep them separate in reports."""
...
def build_financial_charts(transactions: list[dict]) -> dict:
    ...
    for tx in sorted_transactions:
        ...
        if tx_type == "expense":
            ...
            if category in {"housing", "utilities", "groceries", "healthcare", "insurance", "debt_loans", "fees", "government_taxes"}:
...
# Optional local smoke test. Safe to remove in production.
if __name__ == "__main__":
    samples = [
        "ايداع رواتب القوات البريه الملكيه السعوديه /PAYROLL/",
        "مدفوعات سداد الكهرباء",
        "Pay Apple - شراء عبر نقاط بيع ستاربكس RIYADH",
        "حواله فوريه محليه صادره مصاريف سفر",
        "خصم قسط قرض شخصي",
        "رسوم تحويل CITY:Digital Channel",
    ]
