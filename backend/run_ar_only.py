...
def main() -> int:
    ...
    print("TARGET LANGUAGES:", ", ".join(TARGET_LANGUAGES).upper())

    bad_counts = {
        language: len(grouped[language])
        for language in TARGET_LANGUAGES
        if len(grouped[language]) != EXPECTED_PER_LANGUAGE
    }

    if bad_counts:
        print("ERROR: expected 15 AR fixtures")

        for language in TARGET_LANGUAGES:
            paths = grouped[language]
    ...
    global_summary_path = (
        OUTPUT_ROOT / "_summary_ar_only.json"
    )
    ...
    print()
    print("#" * 88)
    print("FINAL ARABIC-ONLY SUMMARY")
    print("#" * 88)
    print("EXPECTED RUNS:", EXPECTED_PER_LANGUAGE * len(TARGET_LANGUAGES))
